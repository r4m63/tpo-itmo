...
public class TrigonometricTest {
    ...
    @ParameterizedTest(name = "arccos({0})") // один и тот же тестовый метод будет вызван много раз с разными входными
                                             // данными
    @DisplayName("Check corner and invalid values") // Красивое имя теста в отчёте
    @ValueSource(doubles = {
    ...
    void checkRandomDots() {
        for (int i = 0; i < 1000; i++) {
            // Генерируем значения строго внутри диапазона, избегая самих 1.0 и -1.0 в
            // рандоме
